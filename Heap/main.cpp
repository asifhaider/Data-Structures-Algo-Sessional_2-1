...
void write_output_file (vector<int>&v)
{
    ofstream output;
    output.open("sorted.txt", ios::app);
    for(int i=0; i<v.size(); i++)
    {
        output << std::to_string(v[i]) << endl;
    }
    output.close();
}
...
int main()
{
    ...
    cout<<h.getMax()<<endl;
    cout << h.size() << endl;
    clock_t heapsort_begins = clock();
    heapsort(numbers);
    cout << "Heap Sort takes " << float(clock() - heapsort_begins)/CLOCKS_PER_SEC << " seconds\n";

    // You need to implement this function in heap.h. You should use the heap class implemented by you to do this. Hint: the function declaration should be void heapsort(vector<int>&v);
    // Now, "numbers" vector contains the numbers in descending order
    write_output_file(numbers);   // writes the output to a separate file
    return 0;
}
